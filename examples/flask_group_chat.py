...
def _create_emotion_agents() -> Dict[str, AssistantAgent]:
    """Instantiate the five Inside Out-style assistant agents."""
    joy = AssistantAgent(
        name="乐乐",
        system_message=(
...
def build_manager(selected: List[str] | None = None) -> tuple[UserProxyAgent, GroupChatManager]:
    """Create a group chat manager with the requested emotion agents."""
    agents = _create_emotion_agents()
    user = UserProxyAgent(
        name="用户",
        human_input_mode="ALWAYS",
        code_execution_config={"use_docker": False},
    )
    chosen = [agents[n] for n in (selected or agents.keys()) if n in agents]
    groupchat = GroupChat(
        agents=[user, *chosen],
        messages=[],
        max_round=6,
...
def index():
    ...
    return send_from_directory(os.path.dirname(__file__), "inside_out_web.html")


@app.get("/bots")
def list_bots():
    """Return available emotion agents."""
    return jsonify({"bots": BOTS})


@app.post("/chat/start")
def start_chat():
    """Start a new chat session using selected agents and return its id."""
    data = request.get_json(force=True)
    selected = data.get("agents") if isinstance(data, dict) else None
    user, manager = build_manager(selected)
